...
function winnerMessage(factionId: FactionId): string {
  const key = `ending.winner${factionId.charAt(0).toUpperCase()}${factionId.slice(1)}` as const;
  return t(key);
}
...
export class EndingScreen implements Screen {
  ...
  private renderElectionResult(): void {
    ...
    const fSuffix = t('charSelect.factionSuffix');
    const orgUnit = t('ending.orgCount');
    ...
    this.container.innerHTML = `
      <div style="max-width:480px; width:100%;">
        ...
        ">
          ...
          <div style="font-size:1.5em; font-weight:bold; color:var(--game-text); margin-bottom:8px;">
            ${isVictory ? t('ending.victory') : t('ending.defeat')}
          </div>
          <div style="color:var(--game-text-dim); font-size:0.9em; margin-bottom:16px;">
            ${this.state.day >= 30
              ? t('ending.activity30')
              : t('ending.activityEarly', { day: this.state.day })}
          </div>
          <div style="
          ...
          ">
            <div style="font-style:italic; color:${winnerFaction?.color ?? 'var(--game-text)'}; margin-bottom:8px;">
              「${winnerMessage(winner.factionId as FactionId)}」
            </div>
          ...
          <div style="
            ...
            font-size:0.85em; color:var(--game-text);
          ">
            <div>${t('ending.winnerFaction')}: <strong>${winner.name}</strong> (${winner.votes}${orgUnit})</div>
            <div>${t('ending.yourFaction')}: <strong>${FACTION_LABELS[this.state.faction ?? 'conservative'] ?? ''}${fSuffix}</strong></div>
            <div>${t('ending.supportOrgs')}: <strong>${supporterCount}</strong>${orgUnit} / ${ORGANIZATIONS.length}${orgUnit}</div>
          </div>
        </div>
          ...
          ${results.map((r, i) => `
            ...
            ">
              ...
              <div style="flex:1; font-size:0.88em; color:var(--game-text);">
                ${r.name}
                ${r.factionId === this.state.faction ? `<span style="color:#4A90D9; font-size:0.8em;"> (${t('ending.supportLabel')})</span>` : ''}
              </div>
              <div style="
        ...
        <div class="game-panel" style="
          margin-bottom:24px;
        ">
          <h3 style="font-size:0.9em; color:var(--game-heading); margin-bottom:10px; font-weight:bold;">${t('ending.orgVoteResult')}</h3>
